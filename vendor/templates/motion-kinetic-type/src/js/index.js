...
[...itemsWrap.querySelectorAll('.item')].forEach(itemEl => {
    ...
    item.DOM.el.tabIndex = 0;
    item.DOM.el.setAttribute('role', 'button');
    item.DOM.el.setAttribute('aria-label', 'Open ' + item.DOM.el.querySelector('h2').textContent);
    item.DOM.el.addEventListener('keydown', event => {
...
const openItem = item => {
    ...
    // gsap timeline
    backCtrl.disabled = true;
    const openTimeline = gsap.timeline({
        onComplete: () => { isAnimating = false; backCtrl.disabled = false; backCtrl.focus(); }
    });
...
const closeItem = () => {
    ...
    const closeTimeline = gsap.timeline({
        onComplete: () => { isAnimating = false; item.DOM.el.focus(); }
    });
    ...
    .add(() => {
        ...
        // Restore focus when the active article is removed, not at the end of
        // a decorative animation which browsers may throttle in background tabs.
