...
| _(achi-a5, claude: shared account + credits)_ | `server/auth.mjs`, `server/platform-billing.mjs`, `server/app.mjs` (AI routes), `server/platform.mjs` (publish route), `server/backend-ai.mjs`, `server/design-ai.mjs`, `src/AccountForm.tsx`, `src/Credits*.tsx`, `src/App.tsx` (header chip), `tests/platform-billing.test.mjs`, `tests/browser/platform-account.spec.ts`, `.env.example` | connecting Plotform to `~/platform` (one account and one credit balance across products): owners sign in through it; paid actions debit by SKU; unpriced SKUs stay free |
