...
| _(claude subagent E: studio-signal)_ | `public/templates/studio-signal/**`, `vendor/templates/studio-signal/**`, `shared/source-templates-studio-signal.json`, `tests/browser/studio-signal.spec.ts`, one import + spread line in `shared/source-templates.mjs` | building the abstract dark record-label edition: kinetic typography, SVG waveform and grid motifs, no photography, release list filtered by format and year plus an animated play-state waveform toggle |
